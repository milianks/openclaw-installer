...
async fn install_openclaw_windows() -> Result<InstallResult, String> {
    let script = format!(
        "{}{}",
        shell::npm_registry_powershell_prelude(),
        r#"
$ErrorActionPreference = 'Stop'
...
async fn install_openclaw_unix() -> Result<InstallResult, String> {
    let script = format!(
        "{}{}",
        shell::npm_registry_bash_prelude(),
        r#"
# 检查 Node.js
...
$registry = $env:OPENCLAW_NPM_REGISTRY
...
Write-Host "使用 npm registry: $registry" -ForegroundColor Green
...
echo "========================================"
echo ""
export npm_config_registry="${OPENCLAW_NPM_REGISTRY:-${NPM_CONFIG_REGISTRY:-https://registry.npmmirror.com}}"
