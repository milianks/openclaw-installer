...
use crate::utils::{platform, security, shell};
...
"#,
            ...
            {
                Err("Windows 暂不支持自动启动终端，请手动运行: openclaw channels login --channel whatsapp".to_string())
            }

            #[cfg(not(target_os = "windows"))]
            {
                Ok(
                    "已在新终端窗口中启动 WhatsApp 登录，请查看弹出的终端窗口并扫描二维码"
                        .to_string(),
                )
            }
        }
...
pub async fn run_security_scan() -> Result<Vec<SecurityIssue>, String> {
    ...
    let port_bind_exposed = {
        ...
        if !exposed {
            ...
            #[cfg(not(target_os = "windows"))]
            {
                if let Ok(output) = shell::run_bash_output(
                    "netstat -tlnp 2>/dev/null | grep 18789 || ss -tlnp 2>/dev/null | grep 18789",
                ) {
    ...
    if has_public_ip {
        issues.push(SecurityIssue {
            ...
            detail: Some(
                "建议使用防火墙限制 18789 端口的外部访问，或者将服务绑定到 127.0.0.1。".to_string(),
            ),
        });
    } else {
    ...
    let has_gateway_token = {
        let env_path = platform::get_env_file_path();
        let env_token = crate::utils::file::read_env_value(&env_path, "OPENCLAW_GATEWAY_TOKEN");
        let config_token = std::fs::read_to_string(platform::get_config_file_path())
            .ok()
        ...
        [config_token, env_token]
            .into_iter()
            .flatten()
            .any(|t| !t.is_empty() && t != shell::DEFAULT_GATEWAY_TOKEN)
    };
    ...
    #[cfg(not(target_os = "windows"))]
    {
        ...
        if let Ok(metadata) = std::fs::metadata(&env_path) {
            ...
            // 环境变量文件不应允许 group/other 读取或写入（建议 600）。
            if perms & 0o077 != 0 {
                issues.push(SecurityIssue {
                    id: "env_file_perms".to_string(),
                    title: "环境变量文件权限过宽".to_string(),
                    description: format!("~/.openclaw/env 文件权限为 {:o}，其他用户可能读取其中的 API Key 等敏感信息。", perms),
                    severity: "medium".to_string(),
                    fixable: true,
                    fixed: false,
                    category: "permissions".to_string(),
                    detail: Some("一键修复会将 ~/.openclaw/env 权限限制为 600。".to_string()),
                });
            }
        }
    }
    ...
    for id in &issue_ids {
        match id.as_str() {
            "no_gateway_token" => {
                ...
                match security::generate_secure_token() {
                    Ok(token) => {
                        let env_path = platform::get_env_file_path();
                        let env_result = crate::utils::file::set_env_value(
                            &env_path,
                            "OPENCLAW_GATEWAY_TOKEN",
                            &token,
                        );
                        let config_result =
                            crate::commands::config::set_gateway_token_in_config(&token);

                        if env_result.is_err() || config_result.is_err() {
                            error!(
                                "[安全修复] 保存 Gateway Token 失败: env={:?}, config={:?}",
                                env_result.err(),
                                config_result.err()
                            );
                    ...
                    Err(e) => {
                        error!("[安全修复] 生成 Gateway Token 失败: {}", e);
                        failed_ids.push(id.clone());
                    }
                }
            }
                ...
                #[cfg(not(target_os = "windows"))]
                {
                    ...
                    match std::fs::metadata(&env_path) {
                        Ok(metadata) => {
                            ...
                            if let Err(e) = std::fs::set_permissions(&env_path, perms) {
                                error!("[安全修复] 修改权限失败: {}", e);
                                failed_ids.push(id.clone());
                                continue;
                            }
